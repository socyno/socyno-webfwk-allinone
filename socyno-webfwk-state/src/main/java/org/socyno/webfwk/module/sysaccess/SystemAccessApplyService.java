...
public class SystemAccessApplyService extends
    ...
    protected void fillExtraFormFields(Collection<? extends SystemAccessApplyFormSimple> forms) throws Exception {
        ...
        if (withDetails.size() > 0) {
            ...
            Map<Long, List<SystemAccessApplyBusinessEntity>> groupedBusinessEntities = flattedBusinessEntities.stream()
                    .collect(Collectors.groupingBy(SystemAccessApplyBusinessEntity::getApplyId));
...
//                
//                if (form.getBusinessEntities() == null) {
//                    List<SystemAccessApplyBusinessEntity> entityList = subSystems.get(form.getId());
//                    for (SystemAccessApplyBusinessEntity accessRequestSubSystemEntity : entityList) {
//                        accessRequestSubSystemEntity
//                                .setBusiness(ClassUtil.getSingltonInstance(FieldSystemBusinessAccessApply.class)
//                                        .queryDynamicValue(accessRequestSubSystemEntity.getBusiness().getId()));
//                        accessRequestSubSystemEntity.setRole(ClassUtil.getSingltonInstance(FieldSystemRole.class)
//                                .queryDynamicValues(new Long[] { accessRequestSubSystemEntity.getRole().getId() }).get(0));
//                    }
//                    form.setBusinessEntities(entityList);
//                }
            }
        }
    }
    ...
    public class EventCreate
        ...
        public StateFormEventResultCreateViewBasic handle(String event, SystemAccessApplyFormDetail originForm,
            ...
            return new StateFormEventResultCreateViewBasic(save(form));
        }
    }
            ...
            for (SystemAccessApplyBusinessEntity entity : originForm.getBusinessEntities()) {
                getFormBaseDao().executeUpdate(SqlQueryUtil.prepareInsertQuery("system_user_scope_role",
                        new ObjectMap().put("=user_id", originForm.getCreatedBy())
                                ...
                                .put("scope_id", entity.isBusinessEntity() ? entity.getBusiness().getId() : "")
        ...
        for (SystemAccessApplyBusinessEntity entity : entities) {
            getFormBaseDao().executeUpdate(SqlQueryUtil.prepareInsertQuery(
                    getBusinessEntityTable(), new ObjectMap()
                            ...
                            .put("business", entity.isBusinessEntity() ? entity.getBusiness().getId() : "")
    ...
    private long save(SystemAccessApplyFormCreation form) throws Exception {
        ...
        if (form instanceof SystemAccessApplyFormEdition) {
            id.set(form.getId());
