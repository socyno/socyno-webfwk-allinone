...
public class FieldSystemFieldClassPath extends FieldTableView {
    ...
    public List<OptionClassPath> queryDynamicOptions(FilterBasicKeyword filter) throws Exception {
        ...
        if (StringUtils.isBlank(filter.getKeyword())) {
            return getFormBaseDao().queryAsList(OptionClassPath.class, String
                    .format("SELECT DISTINCT s.class_path FROM %s s", SystemFieldOptionService.DEFAULT.getFormName()));
        }
    ...
    public List<SystemFieldOptionEntity> queryByClassPath(String... classPath) throws Exception {
        ...
        return getFormBaseDao().queryAsList(
                SystemFieldOptionEntity.class, String.format(SQL_QUERY_CLASS_PATH_IN,
                        SystemFieldOptionService.DEFAULT.getFormTable(), CommonUtil.join("?", classPath.length, ",")),
                classPath);
    }
}
