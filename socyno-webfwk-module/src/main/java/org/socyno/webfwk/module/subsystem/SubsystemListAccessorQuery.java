...
public class SubsystemListAccessorQuery extends AbstractStateFormQuery {
    ...
    protected String requiredAccessEventKey() {
        return SubsystemService.DEFAULT.getFormAccessEventKey();
    }
    
    public SubsystemListAccessorQuery(String keyword, Integer limit, Long page) {
        super(limit, page);
        setKeyword(keyword);
    }
    
    public SubsystemListAccessorQuery(Integer limit, Long page) {
        super(limit, page);
    }
    
    public SubsystemListAccessorQuery(String keyword) {
        this(keyword, null, null);
    }
    
    public SubsystemListAccessorQuery() {
        super();
    }
    ...
    public AbstractSqlStatement buildWhereSql() throws Exception {
        ...
        if (!isDisableIncluded()) {
            ...
            builder.append(String.format(" and s.%s != ?", SubsystemService.DEFAULT.getFormStateField()));
        }
    ...
    public AbstractSqlStatement prepareSqlTotal() throws Exception {
        ...
        return new BasicSqlStatement().setValues(query.getValues()).setSql(
                String.format("SELECT COUNT(1) FROM %s s %s", SubsystemService.DEFAULT.getFormTable(), query.getSql()));
    }
    ...
    public AbstractSqlStatement prepareSqlQuery() throws Exception {
        ...
        return new BasicSqlStatement().setValues(query.getValues())
                .setSql(String.format(SQL_SELECT_FORM + " %s ORDER BY s.code ASC LIMIT %s, %s",
                        SubsystemService.DEFAULT.getFormTable(), query.getSql(), getOffset(), getLimit()));
    }
}
