...
public class FieldApplicationNamespace extends FieldTableView {
    ...
    public static List<OptionApplicationNamespace> queryByApplications(Long... applicationIds) throws Exception {
        ...
        Map<Long, List<OptionApplicationNamespace>> nsapps = new HashMap<>();

        List<OptionApplicationNamespace> samens;
        for (OptionApplicationNamespace oan : list) {
            if ((samens = nsapps.get(oan.getNamespaceId())) == null) {
                nsapps.put(oan.getNamespaceId(), samens = new ArrayList<>());
            }
            samens.add(oan);
        }
        if (nsapps.isEmpty()) {
        ...
        for (OptionDeployClusterNamespace kcn : ClassUtil.getSingltonInstance(FieldDeployNamespace.class)
                .queryDynamicValues(nsapps.keySet().toArray())) {
            for (OptionApplicationNamespace oan : nsapps.get(kcn.getId())) {
