...
@Setter
@ToString
public class DataChartQueryDefault extends AbstractStateFormQuery {
    ...
    public AbstractSqlStatement prepareSqlTotal() throws Exception {
        ...
        return new BasicSqlStatement().setValues(query.getValues()).setSql(String.format(SQL_SELECT_COUNT.concat(" %s"),
                DataChartService.getInstance().getFormTable(), query.getSql()));
    }
    ...
    public AbstractSqlStatement prepareSqlQuery() throws Exception {
        ...
        return new BasicSqlStatement().setValues(query.getValues())
                .setSql(String.format(SQL_SELECT_FORM.concat(" %s ORDER BY s.id DESC LIMIT %s, %s"),
                        DataChartService.getInstance().getFormTable(), query.getSql(), getOffset(), getLimit()));
    }
}
