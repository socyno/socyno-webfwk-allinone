...
public class SystemTenantListDefaultQuery extends AbstractStateFormQuery {
   ...
   public SystemTenantListDefaultQuery(Long page, Integer limit) {
       this(null, page, limit);
   }
   
   public SystemTenantListDefaultQuery(String nameLike, Long page, Integer limit) {
       super(limit, page);
    ...
    private AbstractSqlStatement buildWhereSql() {
        ...
        if (!isDisableIncluded()) {
            ...
            StringUtils.appendIfNotEmpty(sqlcond, " and ").append(SystemTenantService.DEFAULT.getFormStateField())
                    .append(" != ?");
        }
   ...
   public AbstractSqlStatement prepareSqlTotal() {
       ...
       return new BasicSqlStatement().setValues(whereQuery.getValues()).setSql(String.format("%s %s",
                       String.format(SQL_QUERY_COUNT, SystemTenantService.DEFAULT.getFormTable()),
                       whereQuery.getSql()));
   }
   ...
   public AbstractSqlStatement prepareSqlQuery() {
       ...
       return new BasicSqlStatement().setValues(whereQuery.getValues())
               .setSql(String.format("%s %s ORDER BY f.id DESC LIMIT %s, %s",
                       String.format(SQL_QUERY_ALL, SystemTenantService.DEFAULT.getFormTable()),
