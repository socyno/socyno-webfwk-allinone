...
public class SysIssueListDefaultQuery extends AbstractStateFormQuery {
    ...
    public AbstractSqlStatement prepareSqlTotal() {
        ...
        return new BasicSqlStatement().setValues(whereQuery.getValues()).setSql(String.format(
                "SELECT COUNT(1) FROM %s i %s", SysIssueService.DEFAULT.getFormTable(),
                    whereQuery.getSql()));
    }
    ...
    public AbstractSqlStatement prepareSqlQuery() {
        AbstractSqlStatement whereQuery = buidWhereSql();
        return new BasicSqlStatement().setValues(whereQuery.getValues()).setSql(String.format(
                SQL_SELECT_FORM + " %s ORDER BY i.id DESC LIMIT %s, %s",
                SysIssueService.DEFAULT.getFormTable(), whereQuery.getSql(), getOffset(), getLimit()));
    }
}