...
public class SysBuildListDefaultQuery extends AbstractStateFormQuery {
    ...
    public SysBuildListDefaultQuery() {
        super();
    }
    
    public SysBuildListDefaultQuery(long page, int limit) {
        super();
        setPage(page);
        setLimit(limit);
    }
    ...
    private AbstractSqlStatement buildWhereSql() {
        ...
        if (!disableIncluded) {
            ...
            builder.append(String.format(" AND b.%s != ?", SysBuildService.DEFAULT.getFormStateField()));
        }
    ...
    public AbstractSqlStatement prepareSqlQuery() {
        ...
        return new BasicSqlStatement().setValues(whereQuery.getValues()).setSql(
                String.format("%s WHERE %s ORDER BY b.id DESC LIMIT %s, %s",
                        String.format(SQL_QUERY_ALL_BUILDS, SysBuildService.DEFAULT.getFormTable()),
    ...
    public AbstractSqlStatement prepareSqlTotal() {
        ...
        return new BasicSqlStatement().setValues(whereQuery.getValues()).setSql(
                String.format("%s WHERE %s",
                        String.format(SQL_QUERY_COUNT_BUILDS, SysBuildService.DEFAULT.getFormTable()),
                        whereQuery.getSql()));
    }
