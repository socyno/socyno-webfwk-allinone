...
import org.socyno.webfwk.module.deploy.cluster.DeployClusterFormDefaultForm.FieldOptionsState;
...
public class DeployClusterFormDefaultQuery extends AbstractStateFormQuery {
    ...
    public AbstractSqlStatement prepareSqlTotal() {
        ...
        return new BasicSqlStatement().setValues(query.getValues()).setSql(
                String.format("SELECT COUNT(1) FROM %s a %s", DeployClusterService.DEFAULT.getFormTable(),
                        query.getSql()));
    }
    ...
    public AbstractSqlStatement prepareSqlQuery() {
        ...
        return new BasicSqlStatement().setValues(whereQuery.getValues()).setSql(
                String.format(SQL_SELECT_FORM + " %s ORDER BY a.code DESC LIMIT %s, %s",
                        DeployClusterService.DEFAULT.getFormTable(), whereQuery.getSql(), getOffset(), getLimit()));
    }
