...
public class DepartmentQueryDefault extends AbstractStateFormQuery {
    ...
    protected String requiredAccessEventKey() {
        return DepartmentService.getInstance().getFormAccessEventKey();
    }
    
    public DepartmentQueryDefault(String keyword, Integer limit, Integer page) {
        super(limit, page);
        setKeyword(keyword);
    }
    
    public DepartmentQueryDefault(String keyword) {
        this(keyword, null, null);
    }
    
    public DepartmentQueryDefault() {
        super();
    }
    ...
    public AbstractSqlStatement buildWhereSql() throws Exception {
        ...
        if (!isDisableIncluded()) {
            ...
            builder.append(String.format(" and s.%s != ?", DepartmentService.getInstance().getFormStateField()));
        }
    ...
    public AbstractSqlStatement prepareSqlTotal() throws Exception {
        ...
        return new BasicSqlStatement().setValues(query.getValues())
                .setSql(String.format("SELECT COUNT(1) FROM %s s %s" ,
                        DepartmentService.getInstance().getFormTable() , query.getSql()));
    }
