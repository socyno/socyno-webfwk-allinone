...
public class ChangeRequestListDefaultQuery extends AbstractStateFormQuery {
    ...
    @Attributes(title = "状态", type = ChangeRequestSimple.FieldOptionsState.class)
    private String[] states;
    
    @Attributes(title = "部署项名称")
    private String deployItemName;
    
    @Attributes(title = "变更类型", type = ChangeRequestSimple.FieldOptionsChangeType.class)
    private String[] changeTypes;
    
    @Attributes(title = "适用范围", type = ChangeRequestSimple.FieldOptionsScopeType.class)
    private String scopeType;
    
    @Attributes(title = "变更分类", type = FieldChangeRequestCategoryAllAllowed.class)
    private String category;
    
    public ChangeRequestListDefaultQuery() {
        super();
    }
    
    public ChangeRequestListDefaultQuery(long page, int limit) {
        super();
        setPage(page);
        setLimit(limit);
    }
    ...
    public AbstractSqlStatement prepareSqlTotal() throws Exception {
        ...
        return new BasicSqlStatement().setValues(query.getValues())
                .setSql(String.format(SQL_SELECT_COUNT, ChangeRequestService.DEFAULT.getFormTable(), query.getSql()));
    }
    ...
    public AbstractSqlStatement prepareSqlQuery() throws Exception {
        AbstractSqlStatement query = buildWhereSql();
        return new BasicSqlStatement().setValues(query.getValues())
                .setSql(String.format(SQL_SELECT_FORM + " ORDER BY a.id DESC LIMIT %s, %s",
                        ChangeRequestService.DEFAULT.getFormTable(), query.getSql(), getOffset(), getLimit()));
    }
