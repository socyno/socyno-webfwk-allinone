...
public class DynamicFieldOptionQueryDefault extends AbstractStateFormQuery {
    ...
    @Attributes(title = "路径/名称", type = FieldDynamicFieldOptionClassPath.class)
    ...
    public AbstractSqlStatement prepareSqlTotal() throws Exception {
        ...
        return new BasicSqlStatement().setValues(query.getValues()).setSql(String.format(SQL_SELECT_COUNT.concat(" %s"),
                DynamicFieldOptionService.getInstance().getFormTable(), query.getSql()));
    }
    ...
    public AbstractSqlStatement prepareSqlQuery() throws Exception {
        ...
        return new BasicSqlStatement().setValues(query.getValues())
                .setSql(String.format(SQL_SELECT_FORM + " %s GROUP BY s.class_path ORDER BY s.id DESC LIMIT %s, %s",
                        DynamicFieldOptionService.getInstance().getFormTable(), query.getSql(), getOffset(),
                        getLimit()));
    }
}
