...
import com.weimob.webfwk.module.application.ApplicationFormSimple;
...
import com.weimob.webfwk.module.vcs.change.VcsRefsNameOperation.RefsOpType;
import com.weimob.webfwk.module.vcs.common.VcsRevisionEntry;
import com.weimob.webfwk.module.vcs.common.VcsType;
import com.weimob.webfwk.module.vcs.common.VcsUnifiedService;
import com.weimob.webfwk.modutil.authoriy.AuthoriyScopeIdParserFromApplication;
...
//            });
     ...
     */
    private static void checkReleaseModeMatched(VcsRefsNameOperation refsop, ApplicationFormSimple application) {
...
//    }
     ...
     * 属性名称分别为：
     *      weimob-change:next:txn      变更的 TXN 值
     *      weimob-change:next:log      变更描述文本
     *      weimob-change:next:files    变更文件清单
     * 
...
//                    StringUtils.trimToEmpty(getSubversionPropertyAsString(revProps, "weimob-change:next:txn")),
//                    info.getVcsRevision())) {
...
//                    getSubversionPropertyAsString(revProps, "weimob-change:next:log")));
//            refsCommitAppsInfo.setMessage(info.getVcsMessage());
//            /* 确认hook中正确设置了变更文件清单 */
//            if (StringUtils.isBlank(changedFiles = getSubversionPropertyAsString(revProps, "weimob-change:next:files"))) {
...
//    }
    ...
    private void parseGitlabRefsCommitAppsInfo(
                                final VcsChangeInfoFormCreation info,
                                final AbstractSystemTenant tenantInfo,
                                final RefsCommitAppsInfo refsCommitAppsInfo) throws Exception {
        if (StringUtils.isBlank(info.getVcsPath())) {
            throw new MessageException("Gitlab 变更仓库地址必须提供");
        }
        if (StringUtils.isBlank(info.getVcsRefsName())) {
            throw new MessageException("Gitlab 变更分支名称必须提供");
        }
        if (StringUtils.isBlank(info.getVcsRevision())) {
            throw new MessageException("Gitlab 变更后 commit 必须提供");
        }
        if (StringUtils.isBlank(info.getVcsOldRevision())) {
            throw new MessageException("Gitlab 变更前 commit 必须提供");
        }
        List<ApplicationFormSimple> applications;
        if ((applications = ApplicationService.getInstance().queryByVcsPath(info.getVcsPath())) == null
                || applications.size() <= 0) {
            throw new MessageException("应用仓库未在系统中注册，拒绝接收变更");
        }
        RefsOpType refsOpType = info.getVcsOldRevision().matches("^0+$")
                            ? RefsOpType.Create : (
                                    info.getVcsRevision().matches("^0+$") 
                                        ? RefsOpType.Delete 
                                        : RefsOpType.Update
                                );
        VcsRevisionEntry commmitInfo = null;
        ApplicationFormSimple application = applications.get(0);
        /* 非创建操作，需要检查分支或标签其是否存在 */
        if (!RefsOpType.Create.equals(refsOpType)) {
            try {
        ...
        if (!RefsOpType.Delete.equals(refsOpType)) {
            ...
            checkVcsCommitMessage(tenantInfo, VcsType.Gitlab, commmitInfo.getMessage());
            refsCommitAppsInfo.setMessage(commmitInfo.getMessage());
        }
        
        for (ApplicationFormSimple app : applications) {
            VcsRefsNameOperation refsOperation = new VcsRefsNameOperation(
                    app.getName(),
                    info.getVcsPath(),
                    info.getVcsRefsName(),
                    refsOpType,
    ...
    private static String formatToSystemUsername(@NonNull String vcsCommiter, @NonNull String vcsType, @NonNull String vcsPath) {
        // FIXME: 识别VCS用户，匹配系统账户
        return "root@weimob.com";
    }
    
    void submit(@NonNull VcsChangeInfoFormCreation info) throws Exception {
        final String systemUserCode = formatToSystemUsername(info.getVcsCommiter(), info.getVcsType(),
                info.getVcsPath());
    ...
    private void create(VcsChangeInfoFormCreation info) throws Exception {
        ...
        log.info("添加变更集, 萃取的变更集基础信息如下：{}", info);
...
//                .getConfigsNonBlankTrimed("system.vcs.permission.submit.check.skipped.tenants");
        for (VcsRefsNameOperation refsop : refsCommitAppsInfo.getRefsNameOperations()) {
            ApplicationFormSimple application = refsop.getApplicationForm();
...
//            }
    ...
    private class VcsCommittedRevisionFixRunner extends RunableWithSessionContext {
        ...
        public void exec() {
            try {
                ...
                PagedList<?> changeInfos = listForm(QUERIES.DEFAULT, new VcsChangeListDefaultQuery(1, 1L)
                        .setVcsRevision(vcsRevision).setVcsRefsName(vcsRefsName).setApplication(applicationId));
    ...
    public PagedListWithTotal<VcsChangeInfoFormSimple> queryByContextUser(Integer limit, Long page)
            throws Exception {
    ...
    public PagedListWithTotal<VcsChangeInfoFormSimple> queryByContextUser(VcsChangeListContextCond cond, Integer limit,
        ...
        if (cond != null) {
            ...
            query.setApplication(cond.getApplicationId());
    ...
    public PagedListWithTotal<VcsChangeInfoFormSimple> queryByApplication(long applicationId,
            VcsChangeListApplicationCond cond, Integer limit, Long page) throws Exception {
        VcsChangeListDefaultQuery query = new VcsChangeListDefaultQuery(limit, page);
