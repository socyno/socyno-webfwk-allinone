const tool = {
   ...
   */
  baseUrlReplace(url) {
    url = url.replace('http://webfwk.socyno.org', '')
      .replace('https://webfwk.socyno.org', '')
    return url
  },
  ...
  /**
   * 解析 JSON 字串为数据. 当传入两个参数时，如果将第一个参数作为JSON字串解析时失败，则返回第二个参数的值。
   */
  fromJson(json, dflt) {
  ...
  title(subtitle, apply) {
    var title = (window.$title || 'Socyno 后台管理系统') + ' - ' + this.stringify(subtitle)
    if (apply === true) {
   ...
   */
  htmlLineBreak(str) {
    return this.stringify(str).replace(/\r?\n/g, '<br/>')
  },
