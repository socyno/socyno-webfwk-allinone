...
public class ConfigService {
    ...
    private final static Pattern REGEXP_CONFIG_KEY = Pattern.compile(
    ...
    public void setConfig(@NonNull ConfigItem c) throws Exception {
        if ( !REGEXP_CONFIG_KEY.matcher(c.getName()).find() ) {
            throw new InvalidConfigKeyException( String.format(
                    "key = %s", c.getName() ) );
        }
    ...
    private static String[] splitValue(String value, String regex, int flags) {
        value = CommonUtil.ifNull(value, "");
        if ( value.isEmpty() ) {
            return new String[0];
        }
        String[] values = regex == null ? new String[] {value}
                    : value.split(regex);
        if ( flags == 0 ) {
            return values;
        }
