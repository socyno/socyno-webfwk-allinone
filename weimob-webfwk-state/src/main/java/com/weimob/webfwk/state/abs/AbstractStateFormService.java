...
import com.weimob.webfwk.state.exec.StateFormActionDeclinedException;
import com.weimob.webfwk.state.exec.StateFormActionMessageRequiredException;
import com.weimob.webfwk.state.exec.StateFormActionNotFoundException;
import com.weimob.webfwk.state.exec.StateFormActionReturnException;
import com.weimob.webfwk.state.exec.StateFormEventResultNullException;
import com.weimob.webfwk.state.exec.StateFormNotFoundException;
import com.weimob.webfwk.state.exec.StateFormRevisionChangedException;
import com.weimob.webfwk.state.exec.StateFormRevisionNotFoundException;
import com.weimob.webfwk.state.exec.StateFormSubmitEventDefinedException;
import com.weimob.webfwk.state.exec.StateFormSubmitEventTargetException;
...
import com.weimob.webfwk.state.util.StateFormActionDefinition;
import com.weimob.webfwk.state.util.StateFormEventBaseEnum;
import com.weimob.webfwk.state.util.StateFormEventClassEnum;
import com.weimob.webfwk.state.util.StateFormRevision;
import com.weimob.webfwk.state.util.StateFormWithAction;
import com.weimob.webfwk.state.util.StateFormActionDefinition.EventType;
...
@Slf4j
public abstract class AbstractStateFormService<S extends AbstractStateFormBase> {
    ...
    public <T extends S> T getForm(Class<T> clazz, long formId) throws Exception {
       T form;
        ...
        return fillFormBasicField(form, s);
    }
    
    protected <T extends S> T fillFormBasicField(T form, @NonNull StateFormRevision state) {
        if (form != null) {
            form.setState(state.getState());
            form.setRevision(state.getRevision());
            form.setCreatedAt(state.getCreatedAt());
            form.setCreatedBy(state.getCreatedBy());
            form.setCreatedCodeBy(state.getCreatedCodeBy());
            form.setCreatedNameBy(state.getCreatedNameBy());
            form.setUpdatedAt(state.getUpdatedAt());
            form.setUpdatedBy(state.getUpdatedBy());
            form.setUpdatedCodeBy(state.getUpdatedCodeBy());
            form.setUpdatedNameBy(state.getUpdatedNameBy());
        }
        return form;
    }
    ...
    public <T extends S> List<T> listForm(@NonNull Class<T> clazz, @NonNull AbstractStateFormFilter<T> filter) throws Exception {
        ...
        if (list != null && list.size() > 0) {
            ...
            for (S form : list) {
                StateFormRevision s;
                if ((s = srs.get(form.getId())) != null) {
                    fillFormBasicField(form, s);
                }
            }
        }
        return list;
    ...
    public final Map<StateFlowChartNodeData, Set<StateFlowChartNodeData>> parseFormFlowChartDefinition(
            boolean keepUnChanged, AbstractStateFormBase form) throws Exception {
        /**
        ...
        for (Map.Entry<String, AbstractStateAction<S, ?, ?>> e : getFormActions(true, null).entrySet()) {
            ...
            for (String source : action.getSourceStates()) {
                StateFlowChartNodeData sourceNode = new StateFlowChartNodeData(source, getStateDisplay(source), form);
            ...
            parseStateChoice(action.getTargetState(), actionNode, flowNodesData, (S) form);
        }
     ...
     * </pre>
     */
    private void parseStateChoice(final AbstractStateChoice targetChoice,
                                  final StateFlowChartNodeData parentNode,
        ...
        for (Map.Entry<Boolean, AbstractStateChoice> branch : branchTargets.entrySet()) {
            ...
            if (targetChoiceDroped) {
                ...
                parseStateChoice(yesNoChoice, parentNode, flowData, form);
                return;
            }
            ...
            parseStateChoice(yesNoChoice, yesNoFlowNode, flowData, form);
        }
    }
    ...
    public boolean checkAction(String event, S form) throws Exception {
        ...
        try {
            ...
            if (authority.multipleChoice() || authority.multipleOneAllowed()) {
                for (String targetId : allScopeTargetIds) {
     ...
     *
     */
    public boolean checkCreateAction(String event, AbstractStateFormInput form) throws Exception {
        AbstractStateAction<S, ?, ?> action = null;
        if ((action = getExternalFormAction(event)) == null || !EventType.Create.equals(action.getEventType())) {
     ...
     */
    @SuppressWarnings("unchecked")
    public List<String> getActionUserNameWithForm(String event, AbstractStateFormBase form) throws Exception {
     ...
     */
    protected S triggerPreHandle(String event, AbstractStateFormInput form) throws Exception {
        S originForm = null;
        AbstractStateAction<S, ?, ?> action;
        if ((action = getExternalFormAction(event)) == null) {
     ...
     */
    public AbstractStateCreateView triggerCreateAction(String event, AbstractStateFormInput form) throws Exception {
     ...
     * 执行指定的表单创建操作，并返回新表单编号。
     */
    public AbstractStateCreateView triggerCreateAction(String event, AbstractStateFormInput form, String message) throws Exception {
     ...
     * 执行指定的操作，忽略返回值。
     */
    public void triggerAction(String event, AbstractStateFormInput form) throws Exception {
     ...
     */
    public <T> T triggerAction(String event, AbstractStateFormInput form, Class<T> clazz) throws Exception {
     ...
     */
    @SuppressWarnings("unchecked")
    public <T> T triggerAction(final String event, final AbstractStateFormInput form, final String message, final Class<T> clazz) throws Exception {
        AbstractStateAction<S, ?, ?> action;
        if ((action = getExternalFormAction(event)) == null) {
            throw new StateFormActionNotFoundException(getFormName(), event);
        }
    ...
    public abstract void saveMultiChoiceCoveredTargetScopeIds(String event, AbstractStateFormInput form, String scopeType, String ...coverdTargetScopeIds) throws Exception;
    
    public abstract List<Long> queryMultipleChoiceCoveredTargetScopeIds(String event, AbstractStateFormInput form)  throws Exception;;
    
    public abstract void cleanMultipleChoiceTargetScopeIds(String[] clearEvents, AbstractStateFormInput form) throws Exception;;
    
    private void psotHandleForm(String event, String message, Object result, S originForm, AbstractStateFormInput form) throws Exception {
        /**
         * 设置目标状态
         */
        String finalNewState = null;
...
//                }
            ...
            if (!AuthorityScopeIdNoopMultipleParser.class.equals(authority.multipleParser())
                    && authority.multipleChoice()) {
                // 当前用户有权限的业务系统清单
                String[] coverScopeIds = PermissionService.queryFormEventScopeTargetIds(AuthorityScopeType.Business,
                        getFormName(), event);
                // 当前表单涉及的业务系统清单
                String[] multipleScopeTargetIds = createInstance(authority.multipleParser())
                        .getAuthorityScopeIds(originForm == null ? form : originForm);
                String[] finalCoverScopeTargetIds = coverScopeIds != null ? coverScopeIds : multipleScopeTargetIds;
                saveMultiChoiceCoveredTargetScopeIds(event, form, AuthorityScopeType.Business.name(),
                        finalCoverScopeTargetIds);
                ...
                if (allCoverScopeTargetIds == null || allCoverScopeTargetIds.size() <= 0) {
                    finalNewState = originForm.getState();
                }
            }
        ...
        log.info("Form action has been handled : form = {}, id = {}, event = {}, target = {}",
                            getFormName(), form.getId(), event, finalNewState);
        /**
        ...
        if ((isSubmitEvent && form.getId() != null) || originForm != null) {
            ...
            if (originForm != null && (stateActions = getInternalFromActions(originForm.getState())).size() > 0) {
                for (Entry<String, AbstractStateAction<S, ?, ?>> stateAction :stateActions.entrySet()) {
                    if (((eventAction = stateAction.getValue()) instanceof AbstractStateLeaveAction)
                            && (!StringUtils.equals(originForm.getState(), finalNewState)
             ...
             * 处理状态进入事件
             */
            if ((stateActions = getInternalFromActions(finalNewState)).size() > 0) {
                for (Entry<String, AbstractStateAction<S, ?, ?>> stateAction :stateActions.entrySet()) {
                    if (((eventAction = stateAction.getValue()) instanceof AbstractStateEnterAction)
                            && (isSubmitEvent || !StringUtils.equals(originForm.getState(), finalNewState)
         ...
         * 保存状态并回调预定义后处理方法
         */
        triggerPostHandle(event, finalNewState, result, originForm, form, message);
    }
    
    protected void triggerPostHandle(String event, String finalNewState, Object result, S originForm, AbstractStateFormInput form, String message) throws Exception {
        /**
    ...
    protected void triggerExceptionHandle(String event, S originForm, AbstractStateFormInput form, Throwable ex) throws Exception {
    ...
    public final Class<AbstractStateFormBase> getActionOriginTypeClass(String event) throws Exception {
        AbstractStateAction<? extends S, ?, ?> action;
        if((action = getFormAction(event)) == null) {
            throw new StateFormActionNotFoundException(getFormName(), event);
        }
