...
public class ContextUtil {
    ...
    private static String DICTIONARY_MAP_STRING = "system.data.dictionary.";
    ...
    public static void setConfig(String name, String value, String comment)
            throws Exception {
    ...
    public static boolean inDebugMode() {
        return configAccessable() && "yes".equals(
                getConfigTrimed("system.basic.debug.enabled"));
    }

    public static int getSlowSqlMillisecond() {
        final int dfltMs = 1000;
        if (configAccessable()) {
            final int slowMs = CommonUtil.parseInteger(
                    getConfigTrimed("system.base.dao.sql.slow.millisecond"),
                    dfltMs);
    ...
    public static String getDicConfig(String key, String defaultValue) {
        return CommonUtil.ifNull(
                getConfigService().getValue(
                        String.format("%s%s", DICTIONARY_MAP_STRING, key)),
                defaultValue);
    }
    ...
    public static Map<String, Object> getDicConfigs(String key, Map<String, Object> defaultValue) {
        Map<String, Object> map = new HashMap<String, Object>();
        List<String> dictionaryKeys = new ArrayList<String>();
        key = String.format("%s%s.", DICTIONARY_MAP_STRING, key);
        for (String str : getConfigKeys()) {
            if (str.contains(key)) {
                dictionaryKeys.add(StringUtils.substringAfter(str, key));
            }
        }
        ...
        for (String str : dictionaryKeys) {
            map.put(str,
                    getConfigService()
                            .getValue(String.format("%s%s", key, str)));
        }

        return CommonUtil.ifNull(map, defaultValue);
    }
