...
public class LoginTokenUtil {
    ...
    private static long getExpirationMS() {
        return CommonUtil.parseLong(ContextUtil.getConfigTrimed("system.login.session.token.expiration"), 1800000L);
    }
    
    private static String getSecretKey() {
        String loginKey;
        if (StringUtils.isBlank(loginKey = ContextUtil.getConfigTrimed("system.login.token.secret.key"))) {
